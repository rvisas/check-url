...
const showCli = (cliProgram, route, option1, option2) => {
  // const validate = getOption(option1, option2);
  const programOptions = cliProgram.opts();
  return mdLink.mdLinks(programOptions.route, programOptions).then((response) => {
    let output = '';
    ...
    if ((programOptions.stats) && (programOptions.validate)) {
...
showCli(program, route, option1, option2)
